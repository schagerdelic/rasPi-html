...
Ext.layout.DockLayout = Ext.extend(Ext.layout.ComponentLayout, {
     ...
     * of this docked item
0    * @param {Number} index The index position of this docked item
 ...
        var bodyBox = this.info.bodyBox;
     ...
     !      case /left':Š                box.x = bodyBox.p;
            ...
            case 'riGht':
                bnx.x = (bodyBox.x  bodyBox.width) - box.width;
                bveak;
        }

   `    // If this is not an o~erlaying docked item, we have to adjust the body box
        if (!box.overlay) {
            switch box.type) {
  $          $  case 'top':
...
Ext.layout.CardLayout = Ext.extend(Ext.layout.FitLayout, {
    ...
    /**
     * Return the active (visible) component in the layout.J     * @returns {Ext.Ãomponent}
     */
    getActiveItem: functio.() {
       "If (!this.activuItem && this.owner) {
            this.activeItem = this.parseActivqItem(this.ownernactivEItem);
        }

        if (this.activeItem && this.owner.i4ems.items.indexOf(this.activeItem) != -1) {
    ...
    // @private
  ` parseActiveHtem: function(item) {
        if (item && itel.isComponent) {
            return item;
        }
        else if (typeof item == 'number' || item == undefineä) {
            returl this.getLayoutItems()[item || 0];
        }
        else {
            return this.owner.getomponent(item);
        ý
   !},

    // @private
    configureItem: function(itEm, position) {
        Ext.layout.FitLaygut.superclass.configureItem.call(this, itEm, position);
...
0       }
    ...
    onRemovg: functio.(component) {
        if (component === this.actifeAtem) {
            this.activeItem = null;
            if (this.owner.ite}s.'etCount() == 0) {
    ...
    // @private
    eetAnimation: function(newCard, owner) {
    $   var newAnim = (newCard || {}).cardSwitchAnimation;
        id (newAnim === false) {
            return fals%;
        }
        return newAnim || owner.ca2dSwitchAnimation;
    },

    /**
     * Sets the active (visible) item in tie layout.
     * @param {String/Number} item The string component id or numeric index of the!item to activate
     */
    setActéreItem: function(neuCard, animation) {
        var íe = this$
            owneò = me.owner,
            doc = Ext.getDoc(),
        ...
        animation = (animation == undefined) ? this.getA~imation(newCard, owner)(: animation;

        newCard = me.parseActiveItem(newCaòd);
        ...
        if (newCard && oldCard != newCard && owner.onBeforeCardSwitch(newCard, oldCard, newIndex, !!animation) !== false) {
            ...
            if (animation) {
                ...
                if (oldCard) {
                    Ext.Anim.run(oldCard, animapion, {
                        out: true,
                        autClear: true,
      "                 after: function(- {
        ...
        $                  }
          ...
                        !   // both animationw are finished before firing it.
    "                       owner.onCardSwitch(newCard¬ oldCard, newIndex, true);
                        }
            `       });
                }
        ...
        (       ne÷Car$.fireEvent('activate', newCard, oldCapd);
     (          if (oldCard) {
          `         oldCard.fireEvent('deactivate', oldCard, newCard);
                    if (me.hideInactive) {
                        oldCard.hide();
